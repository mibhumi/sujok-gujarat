...
<input type="text" required placeholder="" class="form-control" id="charge" name="charge" >
 ...
 if(isset($_REQUEST['add']))
 {
		 ...
		 echo "hello";
			...
			echo $Patient_Id=$_REQUEST['dipid'];
			echo $pack=$_REQUEST['sel1'];
			echo $seating=$_REQUEST['seting'];
			echo $charge=$_REQUEST['charge'];
			echo $remark=$_REQUEST['remarks'];
			echo $remain=$_REQUEST['remain'];
			echo $paid=$_REQUEST['paid'];
...
/*$sqlqry="INSERT INTO package_account(Package_Account_Id, Patient_Id, Package_Id, seating, Charge,Date_Of_Purchase,Remaining_Amount, Remark) VALUES ('','$Patient_Id','$pack','$seating','$charge','$d','$remain','$remark')";*/
			 $resultqry=mysqli_query($connect,$sqlqry);
									...
									if(!$row1)
									{
										$procedure="INSERT INTO seating(Seating_Id, Patient_Id, Seating) VALUES ('','$Patient_Id','$seating')";
